...
import com.bw.movie.adapter.CinemaLinkAdapter;
import com.bw.movie.adapter.CinemaRegionAdapter;
 ...
 * Description:
 */
public class FragmentCinemaLink extends BaseFragment implements ICinemaContract.IView {
    @BindView(R.id.rv_cinema_nearby_region)
    RecyclerView rvegion;
    @BindView(R.id.rv_cinema_nearby_cinema)
    RecyclerView rvCinema;
    Unbinder unbinder;
    private List<CinemaLinkBean.ResultBean> data;
    ...
    protected int getLayout() {
        return R.layout.fragment_cinema_link;
    }
    ...
    protected void initData() {
        ...
        if (presenter instanceof ICinemaContract.IPresenter) {
            ((ICinemaContract.IPresenter) presenter).getCinemaRegion();
        }
    }

    @Override
    public void onCinemaRegion(CinemaRegionBean cinemaRegionBean) {
        List<CinemaRegionBean.ResultBean> list = cinemaRegionBean.getResult();
        LinearLayoutManager manager = new LinearLayoutManager(getActivity(), RecyclerView.VERTICAL, false);
        final CinemaRegionAdapter adapter = new CinemaRegionAdapter(getActivity(), list);
        rvegion.setLayoutManager(manager);
        rvegion.setAdapter(adapter);
        adapter.Click(new CinemaRegionAdapter.onClick() {
            @Override
            public void setOnClick(int regionId) {
                BasePresenter presenter = getPresenter();
                if (presenter instanceof ICinemaContract.IPresenter) {
                    ((ICinemaContract.IPresenter) presenter).getCinemaLink(regionId);
                }
            }
        });
    ...
    public void onCinemaLink(CinemaLinkBean cinemaLinkBean) {
        data = cinemaLinkBean.getResult();
        LinearLayoutManager manager = new LinearLayoutManager(getActivity(), RecyclerView.VERTICAL, false);
        final CinemaLinkAdapter adapter = new CinemaLinkAdapter(getActivity(), data);
        rvCinema.setLayoutManager(manager);
        rvCinema.setAdapter(adapter);
        adapter.Click(new CinemaLinkAdapter.onClick() {
            @Override
